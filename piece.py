class Piece:
    def __init__(self, shape:list, color ="white" , name = "unknown piece",position=(0,0),angle=0):
        self.name = name
        self.color = color
        self.shape = shape
        self.angle = angle
...
class L_piece(Piece):
    def __init__(self, color="white", position=(0,0),angle=0):
        shape = [["X","X","X"],[".",".","X"],[".",".","."]]
        super().__init__(shape=shape,color=color,name="L piece",position=position,angle=angle)
